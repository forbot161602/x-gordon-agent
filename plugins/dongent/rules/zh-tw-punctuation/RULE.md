...
description: "zh-TW punctuation conventions — on Chinese-led lines in markdown content (documents, commit messages, PRs), convert ASCII , : ; ? to full-width; replace the ellipsis character with ... outside backticks. Script + tests at convert.py / convert_test.py; design rationale at Specification.md."
---
...
At that checkpoint, run [convert.py][convert.py] with `--check` from the file's directory: it reports any punctuation that needs to be full-width and exits non-zero. Convert (run without `--check`) only when it flags something.
