...
- **Before writing**, follow this priority cascade:
  1. The repo's PR template (`.github/pull_request_template.md` or `.github/PULL_REQUEST_TEMPLATE/*`) — follow verbatim if present.
  2. Recent merged PRs as a style reference, **only if they meet basic industry standards**: `gh pr list --state merged --limit 10` then `gh pr view <pr-number>`. If `gh` is unavailable, `git log --grep="^Merge pull request" --merges -n 10` to find merge commits and look up bodies through whatever interface is reachable.
