...
description: Conventional commits format with header / body / footer parts — concise structured header, free body, standard trailers. Messages scoped to the project's reviewers; cross-domain references only when load-bearing.
---

# Commit convention — concise structured project-scoped messages

A commit message is shared prose that future readers see. This rule covers the message's structural form — three parts (header / body / footer) — and its project scope; the content of those parts inherits from the base rules in [Builds on][builds-on].
