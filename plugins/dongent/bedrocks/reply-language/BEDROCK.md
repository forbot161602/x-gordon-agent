...
This covers everything the agent **addresses to the user** — its reasoning, the progress updates and status messages between tool calls, and the final reply. It does **not** govern the **artifacts** produced: those keep their own language (a portable rule library stays English; an existing document keeps its established language). A command may write its output file in English yet report on it in the user's language; the two are independent.
