...
description: Before compaction or any context loss, capture durable session state into the project's agent memory so it survives into future sessions. Write-side.
---

Run before `/compact`, or any time the working context is about to be lost, to persist what must outlive it. Compaction leaves only a lossy auto-summary that survives the current session; this command writes the durable, important facts into the project's agent memory, where they also survive into future sessions.
...
Group by the memory file written, each marked ✨ created or 🔄 updated, with the facts captured into it listed underneath, one terse line each. Skipped facts (already in the durable record) need no entry.
