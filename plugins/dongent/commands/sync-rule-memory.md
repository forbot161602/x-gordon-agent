...
**Keep this file lean.** It loads whole when recalled, so its budget is total size (content, not line count): distil to a line. When a source's below-marker content genuinely grows, move the bulky parts to their own memory file (indexed in `MEMORY.md`) and leave a one-line pointer; what each sync re-derives — the distillation and the Prerequisites answers — stays in the section. Judge by relevance and size, and prune as the rules evolve.
